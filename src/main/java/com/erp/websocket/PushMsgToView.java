...
@Component("pushMsgToView")
@CrossOrigin
@EnableJms
public class PushMsgToView {
    //静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
    private static int onlineCount = 0;
    //concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。
    private static CopyOnWriteArraySet<PushMsgToView> webSocketSet = new CopyOnWriteArraySet<PushMsgToView>();
    ...
    @JmsListener(destination = "pushMsg")
    public void receiveQueue(String text) {
        logger.error("websocket中的jms的text" + text);
        for (PushMsgToView item : webSocketSet) {
            try {
